...
test.describe("Feature 02: Priority System", () => {
  ...
  test("edit todo priority via edit modal", async ({ page }) => {
    ...
    const article = page.locator("article").filter({ hasText: "Priority edit test" });
    await expect(article.getByText("low", { exact: true })).toBeVisible();
    ...
    const updatedArticle = page.locator("article").filter({ hasText: "Priority edit test" });
    await expect(updatedArticle.getByText("high", { exact: true })).toBeVisible();
  });

  test("todos sorted by priority within sections (high → medium → low)", async ({ page }) => {
    await createTodo(page, { title: "Low priority item zz", priority: "low" });
    await createTodo(page, { title: "High priority item zz", priority: "high" });
    await createTodo(page, { title: "Medium priority item zz", priority: "medium" });

    // Get all articles in the Active section
    const activeSection = page.locator("section").filter({ hasText: /Active \(3\)/ });
    const articles = activeSection.locator("article");

    // First article should be high priority
    await expect(articles.nth(0).getByText("high", { exact: true })).toBeVisible();
    // Second should be medium
    await expect(articles.nth(1).getByText("medium")).toBeVisible();
    // Third should be low
    await expect(articles.nth(2).getByText("low", { exact: true })).toBeVisible();
  });
});
